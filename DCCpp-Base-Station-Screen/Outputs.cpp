...
/**********************************************************************
  ...
  DCC ++ BASE STATION admite el control de SALIDA opcional de cualquier Pin sin usar para fines personalizados.
  Los pines se pueden activar o desactivar. El valor predeterminado es establecer los pines ACTIVOS HIGH y los pines INACTIVOS LOW.
  Sin embargo, este comportamiento predeterminado puede invertirse para cualquier pin en cuyo caso ACTIVE = LOW y INACTIVE = HIGH.

  Las definiciones y el estado (ACTIVO / INACTIVO) de los pines se conservan en la EEPROM y se restauran en el encendido.
  El valor predeterminado es configurar cada pin definido como activo o inactivo según su estado restaurado.  
  Sin embargo, el comportamiento predeterminado se puede modificar para que cualquier pin pueda ser forzado a estar activo o inactivo
   en el encendido, independientemente de su estado anterior antes del apagado.
 
  Para que este sketch utilice uno o más pines Arduino como salidas personalizadas, primero defina / edite / elimine
  Se pueden definir las salidas utilizando la siguiente variación del comando "Z":
...
void Output::remove(int n) {
  ...
  if (tt == NULL) {
    INTERFACE.print("<X>");
    Oled::printDelete(3,false); //error al eliminar sensor
    return;
  }
  ...
  switch (sscanf(c, "%d %d %d", &n, &s, &m)) {
    ...
    case 2:                     // argument is string with id number of output followed by zero (LOW) or one (HIGH)
      ...
      if (t != NULL)
        ...
        Oled::printErrorOptionUndefined(3);
        INTERFACE.print("<X>");
      }
      break;

    case 3:                     // El argumento es un string con el identificador id seguido del numero de pin y el flag
      create(n, s, m, 1);
      Oled::printDefined(3, n, s, m); // OLED n=ID s=pin m=flag
